resource "aws_dynamodb_table" "idempotency" {
  ...
  // DynamoDB Streams - needs to be turned on to do replication
  stream_enabled   = true
...
resource "aws_appautoscaling_target" "idempotency_read" {
  ...
  resource_id        = "table/${aws_dynamodb_table.idempotency.id}"
  scalable_dimension = "dynamodb:table:ReadCapacityUnits"
  service_namespace  = "dynamodb"
}

resource "aws_appautoscaling_policy" "idempotency_read" {
  name = "${aws_dynamodb_table.idempotency.id}-autoscaling-read"
...
resource "aws_appautoscaling_policy" "idempotency_write" {
  name = "${aws_dynamodb_table.idempotency.id}-autoscaling-write"
